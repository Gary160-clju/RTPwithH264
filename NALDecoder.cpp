...
typedef struct
{
  int startcodeprefix_len;     //从.h264文件读到的开始前缀 //! 4 for parameter sets and first slice in picture, 3 for everything else (suggested)
  unsigned len;                 //NALU的长度，不包括起始码//! Length of the NAL unit (Excluding the start code, which does not belong to the NALU)
  unsigned max_size;            //! Nal Unit Buffer size
  int forbidden_bit;            //! should be always FALSE     禁止位
  int nal_reference_idc;        //! NALU_PRIORITY_xxxx    值越大，越重要，解码器在解码处理不过来的时候，可以丢掉重要性为0的NALU。
  int nal_unit_type;            //! NALU_TYPE_xxxx       NALU的类型
  char *buf;                    //完整NALU,不包括起始前缀0x000001或0x00000001//! contains the first byte followed by the EBSP
...
int GetAnnexbNALU (NALU_t *nalu)  //ok，已看，得到一个nalu，
{
  int pos = 0;
  ...
  {
    if (feof (bits))//判断是否到了文件尾   遇到文件结束，feof函数值为非零值
    {
...
{
	OpenBitstreamFile("./test.264");//打开264文件，并将文件指针赋给全局变量bits,在此修改文件名实现打开别的264文件。
	NALU_t *n;
	char* nalu_payload;  
	char sendbuf[1500];
	...
	float framerate = 25;    //帧率是15,这个控制的播放速度
	...
	n = AllocNALU(80000);//为结构体nalu_t及其成员buf分配空间。返回值为指向nalu_t存储空间的指针
	

	int nOffsit = 20; //每秒50帧
